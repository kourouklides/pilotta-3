...
public class AIClient {
    ...
    public String play(ArrayList<String> playedCards, String kozi, String teamOfRound) {
        ...
        if (playedCards.size() == 0){ // first to play
            ...
            /* If you belong in the team of the round, 
               then is for your benefit to play Kozi 
             */
            boolean playKozi = false;
            if (this.getID() == 2 ) { // Team
                System.out.println("Bot's team: " + this.team.getName());
                if (teamOfRound.equals(this.team.getName()))
                    playKozi = true;
            } else { // AITeam
                System.out.println("Bot's team: " + this.aiTeam.getName());
                if (teamOfRound.equals(this.aiTeam.getName()))
                    playKozi = true;
            } // else
            
            if (playKozi) { // play kozi
                System.out.println("Beneficial if kozi is played");
                selectedCard = selectKozi(kozi);
                if (selectedCard.equals("")){ // you don't have kozi
                    System.out.println("Bot " + this.getID() + " does not have kozi");
                    
                    /* Select other card - play your ace */
                    int aces = this.analyzeAces(kozi);
                    
                    /* If you don't even hold an ace, then play something else */
                    if (aces == 0) {
                        Card card = playOtherCard(kozi);
                        selectedCard = this.getID() + "_" + card.getFullCardToString();
                        System.out.println("Don't have kozi or ace.");
                        System.out.println("Selected card: " + selectedCard);
                        return selectedCard;
                    } else { // play an ace
                        System.out.println("Playing an ace..");
                        Card selected = playAce(kozi);
                        if (selected != null)
                            selectedCard = selected.getFullCardToString();
                        else
                            System.out.println("Something went wrong -> AIclient, method play");
                    } // else
                } // if
                
                selectedCard = this.getID() + "_" + selectedCard;
                System.out.println("Full card: " + selectedCard);
                return selectedCard;
            } else { // not beneficial to play kozi
                System.out.println("Not beneficial to play kozi. play ace or smth else");
                /* Better to play ace */
                int aces = this.analyzeAces(kozi);
                ...
                if (aces == 0) {
                    Card card = playOtherCard(kozi);
                    System.out.println("aces == 0: card:" + card.getFullCardToString());
                    selectedCard = this.getID() + "_" + card.getFullCardToString();
                    System.out.println("Don't have kozi or ace.");
                    System.out.println("Selected card: " + selectedCard);
                    return selectedCard;
                    ...
                    Card selected = playAce(kozi);
                    if (selected != null)
                        selectedCard = this.getID() + "_" + selected.getFullCardToString();
                    else
                        System.out.println("Something went wrong -> AIclient, method play");
                } // else                
            } // else
    ...
    private String playKoziNoSuit(String suit, String kozi, ArrayList<String> playedCards) {
        ...
        if (kozi.equals("Hearts")) {
                ...
                if (playedCards.size() > 1) {
                   ...
                   if (currentMax == null) {
                       ...
                       System.out.println("There are kozia on table");
                                              /* Added in simulations */
                       String c = currentMax.split("_")[1];
                       
                       Card highestCardOnTable = new Card(c, new Suit(kozi));
                       Card yourCard = null;
                       int yourCardIndex = -1;
            ...
            if (this.diamondsCards.size() == 0) {
                ...
                if (playedCards.size() > 1) {
                   ...
                   if (currentMax == null) {
                       ...
                       System.out.println("There are kozia on table");
                       System.out.println("Debugging currentMax: " + currentMax);
            ...
            if (this.spadesCards.size() == 0) {
                ...
                if (playedCards.size() > 1) {
                   ...
                   if (currentMax == null) {
                       ...
                       Card highestCardOnTable = new Card(c, new Suit(kozi)); // it was new Card(currentMax, new Suit(kozi))
                       Card yourCard = null;
                       int yourCardIndex = -1;
            ...
            if (this.heartsCards.size() != 0) {
                card = this.getID() + "_" + this.heartsCards.get(0).getFullCardToString();
                this.heartsCards.remove(0); 
            } else if (this.diamondsCards.size() != 0) {
                card = this.getID() + "_" + this.diamondsCards.get(0).getFullCardToString();
                this.diamondsCards.remove(0);
            } else if (this.spadesCards.size() != 0) {
                card = this.getID() + "_" + this.spadesCards.get(0).getFullCardToString();
                ...
                card = this.getID() + "_" + "Seven_Hearts";

            return card;
        } // else
        
        return (this.getID() + "_" + "Seven_Hearts");
    } // playKoziNoSuit
    ...
    private Card playOtherCard(String suit){
        ...
        if (suit.equals("Hearts")){
            if (this.diamondsCards.size() != 0) {
                ...
                //card = null;
               System.err.println("Something went wrong - method playOtherCard - class AIClient - Hearts");
            ...
            if (this.heartsCards.size() != 0) {
                ...
                System.err.println("Something went wrong - method playOtherCard - class AIClient - Diamonds");
                //card = this.diamondsCards.get(0);
                //this.diamondsCards.remove(0);
                card = new Card("Eight", new Suit("Hearts"));
                //card = null;
            } // else            
        } else if (suit.equals("Spades")){
            if (this.diamondsCards.size() != 0) {
                ...
                System.err.println("Something went wrong - method playOtherCard - class AIClient - Spades");
            ...
            if (this.diamondsCards.size() != 0) {
                ...
                System.err.println("Something went wrong - method playOtherCard - class AIClient - Clubs");
                //card = null;
                card = this.clubsCards.get(0);
                this.clubsCards.remove(0);
            } // else            
